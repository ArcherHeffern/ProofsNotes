...
an \textbf{element of} a set is an object that is contained in the set. If x is an element of set S, then we say\begin{align*}
	...
	If x is not an element of set S, then we say\begin{align*}
		...
		\ex{Subsets and Elements of}{
			Given\ {S = \{\text{All countries in europe}\} = \{\text{France, Germany..}\}}\ and\\
			{T = \{\text{All countries in the world}\} = \{\text{France, Germany, China, USA..}\}}
			\begin{align*}
			{S \subseteq T}\end{align*}
			\begin{align*}
		...
		\qs{}{Find all subsets of \{ 2, 5, 8 \}}
		\sol \begin{align*}
			\{\emptyset, \{2\}, \{5\}, \{8\}, \{2, 5\}, \{2, 8\}, \{5, 8\}, \{2, 5, 8\}\}\end{align*}
		\qs{}{If A has N elements, how many subsets does it have?}
		\sol \begin{align*}
			2^N\end{align*}
